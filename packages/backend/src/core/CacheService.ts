...
export class CacheService implements OnApplicationShutdown {
	...
	constructor(
		...
		this.quoteMutingsCache = new RedisKVCache<Set<string>>(this.redisClient, 'quoteMutings', {
			lifetime: 1000 * 60 * 30, // 30m
			memoryCacheLifetime: 1000 * 60, // 1m
			fetcher: (key) => this.quoteMutingsRepository.find({ where: { muterId: key }, select: { muteeId: true } }).then(xs => new Set(xs.map(x => x.muteeId))),
			toRedisConverter: (value) => JSON.stringify(Array.from(value)),
			fromRedisConverter: (value) => new Set(JSON.parse(value)),
		});
		...
		this.avatarDecorationMutingsCache = new RedisKVCache<Set<string>>(this.redisClient, 'avatarDecorationMutings', {
			lifetime: 1000 * 60 * 30, // 30m
			memoryCacheLifetime: 1000 * 60, // 1m
			fetcher: (key) => this.avatarDecorationMutingsRepository.find({ where: { muterId: key }, select: { muteeId: true } }).then(xs => new Set(xs.map(x => x.muteeId))),
			toRedisConverter: (value) => JSON.stringify(Array.from(value)),
			fromRedisConverter: (value) => new Set(JSON.parse(value)),
		});
