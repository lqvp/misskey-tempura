			...
			try {
				if (includeAnnounce && activity.type === 'Announce') {
					...
					try {
						...
						if (renote === null) {
							renote = await this.apNoteService.createNote(activity.object, undefined, this.apResolverService.createResolver());
							if (renote === null) {
					...
					if (object) continue;
					await this.apNoteService.createNote(activity.object, undefined, this.apResolverService.createResolver());
				}
			} catch (err) {
