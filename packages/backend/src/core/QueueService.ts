...
const REPEATABLE_SYSTEM_JOB_DEF = [{
	...
	name: 'cleanExpiredMultipartUploads',
	// 毎時0分に起動
	pattern: '0 * * * *',
}];
...
export class QueueService {
	 ...
	 */
	@bindThis
	public scheduledNoteDelete(noteId: string, fileIds: string[] | undefined, deleteAt: number): Promise<Bull.Job> {
		const delay = deleteAt - Date.now();
		return this.scheduledNoteDeleteQueue.add(noteId, {
			noteId,
			fileIds,
		}, {
			delay,
