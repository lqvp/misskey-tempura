...
export function genOpenapiSpec(config: Config, includeSelfRef = false) {
	const spec = {
		...
		externalDocs: {
			...
			url: 'https://github.com/lqvp/misskey-tempura',
		},
	...
	for (const endpoint of copiedEndpoints) {
		...
		const info = {
			...
			externalDocs: {
				...
				url: `https://github.com/lqvp/misskey-tempura/blob/develop/packages/backend/src/server/api/endpoints/${endpoint.name}.ts`,
			},
			...(endpoint.meta.tags ? {
