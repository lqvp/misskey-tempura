...
import type { FlashLikesRemoteRepository, FlashsRepository, MiUser, UsersRepository } from '@/models/_.js';
...
export default class extends Endpoint<typeof meta, typeof paramDef> {
	...
	async remote(
		...
		const remote_user_id = await fetch_remote_user_id(this.config, this.httpRequestService, this.redisForRemoteApis, user);
		if (remote_user_id === null) {
			return [];
		}
		const remote_json = await fetch_remote_api(this.config, this.httpRequestService, user.host, '/api/users/flashs', { userId: remote_user_id, limit, sinceId, untilId });
