...
export default class extends Endpoint<typeof meta, typeof paramDef> { // eslint-disable-line import/no-default-export
	constructor(
		...
		super(meta, paramDef, async (ps, me) => {
			// Query to retrieve follow requests sent by the current user
			const query = this.queryService.makePaginationQuery(this.followRequestsRepository.createQueryBuilder('request'), ps.sinceId, ps.untilId)
				.andWhere('request.followerId = :meId', { meId: me.id }); // Filter by followerId (the current user)

			const requests = await query
				.limit(ps.limit)
				.getMany();

			// Pack and return the requests
