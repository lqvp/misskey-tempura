						...
						<SearchMarker :keywords="['default', 'note', 'visibility']">
							<MkDisableSection :disabled="rememberNoteVisibility">
								<MkFolder>
									...
									<template v-else-if="$i.policies.canPublicNonLtlNote && defaultNoteVisibility === 'public_non_ltl'" #suffix>{{ i18n.ts._visibility.public_non_ltl }}</template>
									...
									<div class="_gaps_m">
										<MkPreferenceContainer k="defaultNoteVisibility">
											<MkSelect
												...
												:items="defaultNoteVisibilityItems"
											>
											</MkSelect>
										</MkPreferenceContainer>
...
const defaultNoteVisibilityItems = computed(() => {
	const items = [
	...
	if ($i.policies.canPublicNonLtlNote) {
		items.splice(1, 0, { label: i18n.ts._visibility.public_non_ltl, value: 'public_non_ltl' });
	}
	return items;
});
