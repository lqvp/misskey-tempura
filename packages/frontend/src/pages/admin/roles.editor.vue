	...
	<FormSlot>
		...
		<div class="_gaps_s">
			...
			<MkFolder v-if="matchQuery([i18n.ts._role._options.remoteLocalTimelineAnyLimit, 'remoteLocalTimelineAnyLimit'])">
				<template #label>{{ i18n.ts._role._options.remoteLocalTimelineAnyLimit }}<span class="_beta">{{ i18n.ts.originalFeature }}</span></template>
				<template #suffix>
					...
					<span v-else>{{ role.policies.remoteLocalTimelineAnyLimit.value ? i18n.ts.yes : i18n.ts.no }}</span>
				...
				<div class="_gaps">
					...
					<MkSwitch v-model="role.policies.remoteLocalTimelineAnyLimit.value" :disabled="role.policies.remoteLocalTimelineAnyLimit.useDefault" :readonly="readonly">
						<template #label>{{ i18n.ts.enable }}</template>
					</MkSwitch>
					<MkRange v-model="role.policies.remoteLocalTimelineAnyLimit.priority" :min="0" :max="2" :step="1" easing :textConverter="(v) => v === 0 ? i18n.ts._role._priority.low : v === 1 ? i18n.ts._role._priority.middle : v === 2 ? i18n.ts._role._priority.high : ''">
						<template #label>{{ i18n.ts._role.priority }}</template>
					</MkRange>
				</div>
			</MkFolder>
