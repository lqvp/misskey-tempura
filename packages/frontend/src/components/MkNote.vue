	...
	<article v-else :class="$style.article" @contextmenu.stop="onContextmenu">
		...
		<div :class="$style.main">
			...
			<footer :class="$style.footer">
				...
				<button v-if="appearNote.reactionAcceptance !== 'likeOnly' && $appearNote.myReaction == null && prefer.s.showLikeButton" ref="heartReactButton" v-tooltip="i18n.ts.like" :class="$style.footerButton" class="_button" @click="handleToggleHeartReact()">
...
import { inject, ref, useTemplateRef, provide, computed, onMounted } from 'vue';
...
const { add: addViewedRenote } = useViewedRenotes();
...
function handleToggleHeartReact() {
	toggleHeartReact((reaction) => {
		if ($appearNote.myReaction === reaction) {
			emit('removeReaction', reaction);
		} else {
			emit('reaction', reaction);
