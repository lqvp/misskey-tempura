		...
		switch (token.type) {
			...
			case 'fn': {
				...
				switch (token.props.name) {
					...
					case 'sparkle': {
						...
						return h(MkSparkle, {}, { default: () => genEl(token.children, { scale, disableNyaize, disableRjNumber }) });
					}
					case 'rotate': {
			...
			case 'link': {
				return [h(MkLink, {
					...
					rel: 'nofollow noopener',
					navigationBehavior: props.linkNavigationBehavior,
				}, { default: () => genEl(token.children, { scale, disableNyaize: true, disableRjNumber: true }) })];
			}
