...
<div :class="$style.root">
	<div :class="$style.head">
		...
		<MkAvatar v-else-if="['roleAssigned', 'achievementEarned', 'exportCompleted', 'login', 'loginFailed', 'createToken', 'scheduledNoteFailed', 'cleanupDanglingFollowsCompleted'].includes(notification.type)" :class="$style.icon" :user="$i" link preview/>
		...
		<div
			:class="[$style.subIcon, {
				...
				[$style.t_cleanupDanglingFollowsCompleted]: notification.type === 'cleanupDanglingFollowsCompleted',
			}]"
		>
			...
			<i v-else-if="notification.type === 'cleanupDanglingFollowsCompleted'" class="ti ti-user-bolt"></i>
			<MkReactionIcon
	...
	<div :class="$style.tail">
		<header :class="$style.header">
			...
			<span v-else-if="notification.type === 'cleanupDanglingFollowsCompleted'">{{ i18n.ts._notification.cleanupDanglingFollowsCompleted }}</span>
		...
		<div>
			...
			<MkA v-else-if="notification.type === 'cleanupDanglingFollowsCompleted'" :class="$style.text">
				<Mfm :text="i18n.tsx._notification.cleanupDanglingFollowsCompletedDescription({ count: notification.count })"/>
			</MkA>
...
.t_follow, .t_followRequestAccepted, .t_receiveFollowRequest, .t_unblocked {
...
.t_cleanupDanglingFollowsCompleted {
	background: var(--eventOther);
	pointer-events: none;
}
