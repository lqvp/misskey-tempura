...
export function useNote(
	...
	const appearNote = normalizeNote(getAppearNote(rawNote) ?? rawNote);

	// 閲覧済みリノートの記録
	const { has: isViewedRenote } = useViewedRenotes();
	...
	async function heartReact(customMockCallback?: (reaction: string) => void) {
		...
		if (props.mock) {
			if (customMockCallback) customMockCallback(prefer.s.selectReaction);
			return;
		}
	...
	function toggleHeartReact(customMockCallback?: (reaction: string) => void) {
		if ($appearNote.myReaction == null) {
			heartReact(customMockCallback);
		} else {
			if (props.mock && customMockCallback) {
				customMockCallback($appearNote.myReaction);
			} else {
